...
func (a *Application) fakeAgentLoop() {
	send := func(e model.Event) { a.EventCh <- e }
	...
	sleep(1500)
	send(model.Event{Type: model.AgentReply, Message: "I'll check the accuracy on the fine-tuned Qwen 7B model. Let me find the eval config first."})
	sleep(600)
	...
	sleep(300)
	send(model.Event{Type: model.AgentReply, Message: "Found the eval config. Running the benchmark now."})
	sleep(600)
	send(model.Event{Type: model.CmdStarted, Message: "python eval.py --model qwen-7b-ft --dataset mmlu"})
	sleep(1500)
	...
	send(model.Event{Type: model.AgentThinking})
	sleep(1500)
	send(model.Event{Type: model.AnalysisReady, Message: "Accuracy is 84.7% on MMLU, 2.3% above baseline. F1 is 83.9%. The fine-tuned model looks good."})
	sleep(1000)

	// Task 2: fix OOM in training loop
	send(model.Event{Type: model.AgentThinking})
	sleep(1800)
	send(model.Event{Type: model.AgentReply, Message: "Now I'll investigate the OOM issue in the training loop."})
	sleep(600)
	...
	sleep(300)
	send(model.Event{Type: model.AgentReply, Message: "Found the leak in model/layer3.go line 4. Every forward pass allocates a new []float32 buffer that never gets reused. I'll profile to confirm."})
	sleep(800)
	send(model.Event{Type: model.CmdStarted, Message: "go tool pprof -alloc_space mem.prof"})
	sleep(1000)
	...
	send(model.Event{Type: model.AgentReply, Message: "Confirmed: allocTensor leaks 512MB per step. Applying sync.Pool fix."})
	sleep(800)
	send(model.Event{Type: model.ToolEdit, Message: "model/layer3.go\n\n-    buf := make([]float32, l.size)\n+    buf := l.pool.Get(l.size)\n+    defer l.pool.Put(buf)"})
	sleep(800)
	send(model.Event{Type: model.AgentReply, Message: "Patched. Running memory test to verify."})
	sleep(600)
	send(model.Event{Type: model.CmdStarted, Message: "go test -run TestLayer3Memory -memprofile mem2.prof ./model/"})
	sleep(1000)
	...
	sleep(300)
	send(model.Event{Type: model.AnalysisReady, Message: "Memory dropped from 512MB to 12MB per step (97.6% reduction). Fix verified."})
	sleep(1000)

	// Task 3: benchmark inference — with an error
	send(model.Event{Type: model.AgentThinking})
	sleep(1800)
	send(model.Event{Type: model.AgentReply, Message: "Starting inference benchmark across batch sizes."})
	sleep(600)
	send(model.Event{Type: model.ToolRead, Message: "bench/config.yaml", Summary: "15 lines"})
	sleep(300)
	send(model.Event{Type: model.ToolError, ToolName: "Shell", Message: "$ python bench/inference_bench.py --model qwen-7b-ft\n\nTraceback (most recent call last):\n  File \"bench/inference_bench.py\", line 23, in <module>\n    model = load_model(args.model)\n  File \"bench/loader.py\", line 45, in load_model\n    raise RuntimeError(\"CUDA out of memory\")\nRuntimeError: CUDA out of memory. Tried to allocate 2.00 GiB"})
	sleep(1000)
	send(model.Event{Type: model.AgentReply, Message: "CUDA OOM on full load. I'll reduce batch size and use --half precision to fit in memory."})
	sleep(800)
	send(model.Event{Type: model.CmdStarted, Message: "python bench/inference_bench.py --model qwen-7b-ft --half --batch 1,8,32,128"})
	sleep(800)
	...
	sleep(300)
	send(model.Event{Type: model.ToolWrite, Message: "bench/production.yaml\n\n+ model: qwen-7b-ft\n+ precision: fp16\n+ batch_size: 32\n+ max_vram_gb: 8\n+ throughput_target: 1500"})
	sleep(800)
	send(model.Event{Type: model.AgentThinking})
	sleep(1500)
	send(model.Event{Type: model.AgentReply, Message: "Throughput plateaus at batch=128 (1637 tok/s) due to memory bandwidth. Batch=32 gives the best tradeoff: 1520 tok/s at 8GB VRAM."})
	sleep(800)
	send(model.Event{Type: model.AgentReply, Message: "Created bench/production.yaml with recommended settings. Use batch_size=32 with fp16 for production."})
	sleep(600)
